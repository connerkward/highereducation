...
SubShader {
	...
	Pass {
		...
		uniform half _MoonTex;
		uniform half _HorizonBrightness;
		uniform half3 _NightSkyTint;
		uniform half _SunHDR; // Sun HDR
		...
		//Adjusts the brightness of the horizon!!!!!!!!!!!!!
		#define OUTER_RADIUS _HorizonBrightness
...
//				for(int i=0; i<int(kSamples); i++)
				...
				OUT.cIn.xyz = frontColor * (kInvWavelength * kKrESun); //Maybe
		...
		half4 frag (v2f IN) : SV_Target
		{
			...
			if(IN.rayDir.y < 0.02)
			{
				...
				if(IN.rayDir.y < 0.0)
				{
					...
					col += mie * IN.cOut * _LightColor0.xyz * (exp(_SunHDR));
				}
				else
				{
