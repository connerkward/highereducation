...
public class PlayerVitalsExample_C_Mobile : MonoBehaviour {
	...
	if (uniStormSystem != null)
	{
		...
		if (uniStormSystemScript.temperature >= 51 && !isSheltered && uniStormSystemScript.currentRainIntensity <= 0)
		{
			warmth += Time.deltaTime * 0.5f;
			...
			if (uniStormSystemScript.currentRainIntensity >= 50 && !isSheltered || uniStormSystemScript.currentSnowIntensity >= 50 && !isSheltered)	
			{
